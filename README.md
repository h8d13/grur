...
`grimaur` or `grur` for short is a unified package management wrapper for Arch Linux that combines pacman operations, AUR package building, Flatpak and an interactive TUI into a single tool. **With advanced features** like git fallback, ssh, maintenance tools, etc.
